///ds_vector_mult_vector( id: n, source: m )
/*
 * Multiplies a vectors values given its ID with another vector.
 * Arguments:
 ...
 */
{
    ...
    if ( otherData == -1 ) {
        _ds_vector_error( concat( 'Cannot multiply vector ( ', argument0, ' ). Other vector ( ', argument1, ' ) does not exist or is invalid.' ), false );
        return -1;
    }
    
    var size = ds_list_size( data );
    if ( size != ds_list_size( otherData ) ) {
        _ds_vector_error( concat( 'Cannot multiply vector ( ', argument0, ' ). Needed ', size, ' dimensions, but was given ', ds_list_size( otherData ), '.' ), false );
        return -1;
    }
