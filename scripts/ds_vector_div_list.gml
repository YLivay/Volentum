///ds_vector_div_list( id: n, list: ln )
/*
 * Divides a vectors values given its ID with a list.
 * Arguments:
 ...
 */
{
    ...
    if ( data == -1 ) {
        ...
        _ds_vector_error( concat( 'Cannot divide vector (', argument0, '). List does not exist or is invalid.' ), false );
        return -1;
    }
    
    var size = ds_list_size( data );
    if ( size != ds_list_size( argument1 ) ) {
        _ds_vector_error( concat( 'Cannot divide vector (', argument0, '). Needed ', size, ' dimensions, but was given ', ds_list_size( argument1 ), '.'), false );
        return -1;
    }
