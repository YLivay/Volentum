///ds_vector_mult_array( id: n, array: n[] )
/*
 * Multiplies a vectors values given its ID with an array.
 * Arguments:
 ...
 */
{
    ...
    if ( data == -1 ) {
        ...
        _ds_vector_error( concat( 'Cannot multiply vector ( ', argument0, ' ). Array does not exist or is invalid.' ), false );
        return -1;
    }
    
    var size = ds_list_size( data );
    if ( size != array_length_1d( argument1 ) ) {
        _ds_vector_error( concat( 'Cannot multiply vector ( ', argument0, ' ). Needed ', size, ' dimensions, but was given ', array_length_1d( argument1 ), '.' ), false );
        return -1;
    }
