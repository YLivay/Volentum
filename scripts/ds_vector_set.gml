///ds_vector_set( id: n, num1, num2, num3... )
/*
 * Sets a vectors values given its ID.
 * This function is limited for 15 dimension vectors, if you need more:
 * use ds_vector_set_array, ds_vector_set_list or ds_vector_set_vector
 * Arguments:
 ...
 */
{
    ...
    if ( data == -1 ) {
        _ds_vector_error( concat( 'Cannot set to vector (', argument[0], '). Vector does not exist.' ), false );
        return -1;
    }
    else if ( ds_list_size( data ) != argument_count - 1 ) {
        _ds_vector_error( concat( 'Cannot set to vector (', argument[0], '). Needed ', ds_list_size( data ), ' dimensions, but was given ', argument_count - 1, '.' ), false );
        return -1;
    }
